...
scripts="$home/SCRIPTS/add_complete_information";
...
join_identical_plus_features="1";
if [ "$join_identical_plus_features" == "1" ]; then
   ...
   python $scripts/FinalJoiningStep.py \
          $all_scores_home/$input_filename.phylocsf/all_joined_scores_by_uORF.average.location.with_header.tsv  \
          $all_scores_home/$input_filename.phylocsf/${input_filename}_complete_uORFs_with_exp_homATG.tsv

#          $all_scores_home/$input_filename.phylocsf/all_joined_scores_by_uORF_cleaned.average.location.with_header.tsv  \
