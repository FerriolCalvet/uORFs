...
for (prior in c(seq(0.25, 0.95, 0.1))){
  ...
  #write.predictions
  unl_list = write.predictions_unlabelled(unl.looping.disc, unl.looping.disc, pos.looping.disc, max.len, prior,
                                          it_max = 30,
                                          quantile_threshold = 0.99,
                                          filename=paste("uORF_function_unlabeled_predictions_sorted_discretized_looping", i, prior, "including", sep='_'))
  unl.pred.file = unl_list[[1]]
  u.prbb = unl_list[[2]]
  p.prbb = unl_list[[3]]
  pos.pred.file = write.predictions(pos.looping.disc, u.prbb, p.prbb, prior, filename=paste("uORF_function_positive_predictions_sorted_discretized_looping", i, prior,"including", sep='_'))
  ret.pred.file = write.predictions(ret.looping.disc, u.prbb, p.prbb, prior, filename=paste("uORF_function_retained_predictions_sorted_discretized_looping", i, prior,"including", sep='_'))
...
for (i in 1:8) {
  catfile.unl = paste("uORF_function_unlabeled_predictions_sorted_discretized_looping", i,prior,"including", sep='_')
  cat.unl.add = read.table(catfile.unl, header=FALSE,sep="\t",comment.char="", quote="")
  cat.unl = rbind(cat.unl,cat.unl.add)
  catfile.pos = paste("uORF_function_positive_predictions_sorted_discretized_looping", i, prior,"including", sep='_')
  cat.pos.add = read.table(catfile.pos, header=FALSE,sep="\t",comment.char="", quote="")
  cat.pos = rbind(cat.pos,cat.pos.add)
  catfile.ret = paste("uORF_function_retained_predictions_sorted_discretized_looping", i, prior,"including", sep='_')
...
svg('ret_pos_unl_distro_1_0.99_8_including.svg')
par(mfrow = c(3,1))
# xlim = c(1.00, 1000)
xlim = c(-100, 100)
...
hist.looping(cat.unl[,2], xlim[1], xlim[2], breaks=40, main="Class probability ratio distribution for unlabeled uORFs", col="lightgreen")
